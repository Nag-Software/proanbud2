...
const MODULE_MISSING_ERROR =
  "Timeføring er ikke aktivert. Gå til abonnement for å aktivere modulen."
...
export async function getActiveWorkSessionAction(
  ...
  try {
    ...
    if (!(await hasTimeforingModule(companyId))) return { ok: true, data: null }
    ...
    if (error) {
      ...
      return {
        ok: false,
        error: "Kunne ikke hente den aktive arbeidsøkten. Last siden på nytt og prøv igjen.",
      }
    }

    return { ok: true, data: (data as WorkSession | null) ?? null }
  } catch (error) {
    await logServerError({
      message: "Uventet feil ved henting av aktiv arbeidsøkt",
      error,
      source: "action",
      route: "getActiveWorkSessionAction",
      context: { projectId },
...
export async function startWorkSessionAction(
  ...
  try {
    ...
    const { companyId } = await getEffectiveRole(supabase, user.id)
    if (!companyId) {
      return { ok: false, error: "Fant ikke bedriften din. Last siden på nytt og prøv igjen." }
    }

    if (!(await hasTimeforingModule(companyId))) {
      return { ok: false, error: MODULE_MISSING_ERROR }
    }
    ...
    if (existingActive) {
      if (existingActive.project_id === projectId) {
        return { ok: false, error: "Du har allerede en aktiv arbeidsøkt på dette prosjektet" }
      }
      ...
      return {
        ok: false,
        error: `Du har allerede en aktiv arbeidsøkt${project?.name ? ` på «${project.name}»` : ""}. Avslutt den først.`,
      }
    }
    ...
    if (error) {
      ...
      return { ok: false, error: "Kunne ikke starte arbeidsøkten. Prøv igjen om litt." }
    }

    revalidatePath(`/prosjekter/${projectId}`)
    revalidatePath("/min-bedrift/timeforing")
    return { ok: true, data: data as WorkSession }
  } catch (error) {
    await logServerError({
      message: "Uventet feil ved start av arbeidsøkt",
      error,
      source: "action",
      route: "startWorkSessionAction",
      context: { projectId },
...
export async function geofenceCheckInAction(
  ...
  try {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
      return { ok: false, error: "Fikk ikke en gyldig posisjon. Slå på stedstjenester og prøv igjen." }
    }
    ...
    if (gf) {
      ...
      if (outsideBy > GEOFENCE_BUFFER_M) {
        return {
          ok: false,
          error: `Du er ikke på byggeplassen${Number.isFinite(outsideBy) ? ` (~${Math.round(outsideBy)} m utenfor)` : ""}. Gå nærmere og prøv igjen.`,
        }
      }
    }
    ...
    if (error) {
      ...
      return { ok: false, error: "Kunne ikke stemple inn. Prøv igjen om litt." }
    }
    ...
    await logServerError({
      message: "Uventet feil ved innstempling (geofence)",
      error,
      source: "action",
      route: "geofenceCheckInAction",
      context: { projectId },
...
export async function stopWorkSessionAction(
  ...
  try {
    ...
    if (activeError || !activeSession?.started_at) {
      return { ok: false, error: "Fant ingen aktiv arbeidsøkt å avslutte" }
    }
    ...
    if (error) {
      ...
      return { ok: false, error: "Kunne ikke avslutte arbeidsøkten. Prøv igjen om litt." }
    }
    ...
    await logServerError({
      message: "Uventet feil ved avslutning av arbeidsøkt",
      error,
      source: "action",
      route: "stopWorkSessionAction",
      context: { projectId },
...
export async function addManualTimeEntryAction(
  ...
  try {
    ...
    if (Number.isNaN(startedAt.getTime()) || Number.isNaN(endedAt.getTime())) {
      return { ok: false, error: "Ugyldig tidspunkt. Sjekk dato, fra og til." }
    }
    ...
    if (diffHours > 24) {
      return { ok: false, error: "En arbeidsøkt kan ikke være lengre enn 24 timer" }
    }

    const hours = Math.round(diffHours * 100) / 100
    if (hours <= 0) {
      return { ok: false, error: "Tidsrommet er for kort" }
    }
    ...
    if (error) {
      ...
      return { ok: false, error: "Kunne ikke lagre timeføringen. Prøv igjen om litt." }
    }
    ...
    await logServerError({
      message: "Uventet feil ved manuell timeføring",
      error,
      source: "action",
      route: "addManualTimeEntryAction",
      context: { projectId },
...
export async function saveCompanyTrackingSettingsAction(
  ...
  try {
    ...
    if (!companyId || !canManageProjects(role)) {
      return { ok: false, error: "Du har ikke tilgang til disse innstillingene" }
    }
    ...
    if (error) {
      ...
      return { ok: false, error: "Kunne ikke lagre innstillingene. Prøv igjen om litt." }
    }
    ...
    await logServerError({
      message: "Uventet feil ved lagring av innstillinger for timeføring",
      error,
      source: "action",
      route: "saveCompanyTrackingSettingsAction",
    })
...
async function setTimeEntryStatus(
  entryId: string,
  ...
  try {
    ...
    const { role, companyId } = await getEffectiveRole(supabase, user.id)
    if (!companyId || !canManageProjects(role)) {
      return { ok: false, error: "Du har ikke tilgang til å godkjenne timer" }
    }
    ...
    if (error) {
      ...
      return { ok: false, error: "Kunne ikke oppdatere statusen. Prøv igjen om litt." }
    }
    ...
    await logServerError({
      message: "Uventet feil ved endring av godkjenningsstatus",
      error,
      source: "action",
      route: "setTimeEntryStatus",
      context: { entryId, status },
    })
    return { ok: false, error: GENERIC_ERROR_MESSAGE }
  }
}

export async function approveTimeEntryAction(entryId: string): Promise<ActionResult<null>> {
  return setTimeEntryStatus(entryId, "approved")
}

export async function rejectTimeEntryAction(entryId: string): Promise<ActionResult<null>> {
  return setTimeEntryStatus(entryId, "rejected")
}

export async function getProjectTimeEntriesAction(
  projectId: string,
  viewAll = false
  ...
  try {
    ...
    const { companyId } = await getEffectiveRole(supabase, user.id)
    if (!(await hasTimeforingModule(companyId))) return { ok: true, data: [] }
    ...
    if (error) {
      ...
      return {
        ok: false,
        error: "Kunne ikke hente timeføringene. Last siden på nytt og prøv igjen.",
      }
    }

    return { ok: true, data: (data || []) as TimeEntryRow[] }
  } catch (error) {
    await logServerError({
      message: "Uventet feil ved henting av timeføringer for prosjekt",
      error,
      source: "action",
      route: "getProjectTimeEntriesAction",
      context: { projectId, viewAll },
    })
    return { ok: false, error: GENERIC_ERROR_MESSAGE }
  }
}
...
export type MyTimeTrackingOverview = {
  activeSession: (WorkSession & { projectName: string }) | null
  projects: MyTimeTrackingProject[]
  recentEntries: MyRecentTimeEntry[]
  weekHours: number
}
 ...
 * aktiv økt (på tvers av prosjekter), valgbare prosjekter, siste 7 dagers
 * egne innslag og timesum for inneværende uke (mandag–søndag, norsk tid).
 * RLS scoper automatisk prosjektlisten for arbeidere.
 */
export async function getMyTimeTrackingOverviewAction(): Promise<
...
> {
  try {
    ...
    // Datovindu i norsk tid: siste 7 dager (inkl. i dag) + start på uka (mandag).
    const todayStr = osloDateString(new Date())
    const todayUtcMidnight = new Date(`${todayStr}T00:00:00Z`)
    const daysSinceMonday = (todayUtcMidnight.getUTCDay() + 6) % 7
    ...
    const [activeResult, projectsResult, recentResult] = await Promise.all([
      // Aktiv økt UTEN prosjektfilter — én aktiv økt per bruker på tvers av prosjekter.
      supabase
        .from("time_entries")
        .select("id, project_id, user_id, started_at, ended_at, description, entry_date, projects(name)")
        .eq("user_id", user.id)
        .is("ended_at", null)
        .maybeSingle(),
      supabase
      ...
      supabase
        .from("time_entries")
        .select("id, entry_date, hours, status, started_at, projects(name)")
        .eq("user_id", user.id)
        .not("ended_at", "is", null)
        .not("hours", "is", null)
        .neq("status", "rejected")
    ...
    const queryError = activeResult.error || projectsResult.error || recentResult.error
    if (queryError) {
      await logServerError({
        message: "Kunne ikke hente timeoversikt for bruker",
        error: queryError,
        source: "action",
        route: "getMyTimeTrackingOverviewAction",
        context: { userId: user.id, companyId },
      })
      return {
        ok: false,
        error: "Kunne ikke hente timeføringen din. Last siden på nytt og prøv igjen.",
      }
    }

    const activeRow = activeResult.data as
      | (WorkSession & { projects?: { name: string } | { name: string }[] | null })
      | null
    const activeSession = activeRow
      ? {
          id: activeRow.id,
          project_id: activeRow.project_id,
          user_id: activeRow.user_id,
          started_at: activeRow.started_at,
          ended_at: activeRow.ended_at,
          description: activeRow.description,
          entry_date: activeRow.entry_date,
          projectName: unwrapRelation(activeRow.projects)?.name || "Ukjent prosjekt",
        }
      : null

    const recentEntries: MyRecentTimeEntry[] = (recentResult.data || []).map((row) => ({
      id: row.id as string,
      entryDate: row.entry_date as string,
      projectName:
        unwrapRelation(row.projects as { name: string } | { name: string }[] | null)?.name ||
        "Ukjent prosjekt",
    ...
    return {
      ...
      data: {
        activeSession,
        projects: (projectsResult.data || []) as MyTimeTrackingProject[],
        recentEntries,
        weekHours,
      },
    }
    ...
    await logServerError({
      message: "Uventet feil ved henting av timeoversikt for bruker",
      error,
      source: "action",
      route: "getMyTimeTrackingOverviewAction",
    })
