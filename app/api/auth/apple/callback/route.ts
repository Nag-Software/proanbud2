...
/**
 * Sign in with Apple callback — mirrors /api/auth/google/callback (code
 * exchange, users bootstrap, pa_oauth_native deep-link handoff). Apple only
 * sends the user's name on the FIRST authorization and has no avatar; the
 * fallbacks below already cover both.
 */
  ...
  try {
    ...
    if (user) {
      ...
      if (process.env.SUPABASE_SERVICE_ROLE_KEY) {
        ...
        if (bootstrapError) {
          console.error('OAuth callback (apple login): users bootstrap failed', bootstrapError)
          await logServerError({
            message: 'Apple login users bootstrap upsert failed',
    ...
    console.error('OAuth callback (apple login) error:', e)
    await logServerError({
      message: 'Apple login OAuth callback failed',
      error: e,
      source: 'api',
      route: 'GET /api/auth/apple/callback',
    })
