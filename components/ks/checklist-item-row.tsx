...
import { CHECKLIST_RESPONSE_LABELS, type ChecklistResponse } from "@/lib/ks/constants"
...
export function ChecklistItemRow({ item, projectId, checklistId, index, onUpdated }: Props) {
  ...
  async function save(nextResponse: ChecklistResponse | null, nextComment?: string) {
    ...
    try {
      ...
      toast.error(actionErrorMessage(err, "Kunne ikke lagre"))
    } finally {
      setSaving(false)
    }
  }
        ...
        <div className="grid grid-cols-3 gap-2">
          {(["ok", "not_ok", "na"] as const).map((val) => (
            <Button
              ...
              className={cn(
                "min-w-0 whitespace-normal px-1 text-center text-sm font-medium leading-tight sm:",
