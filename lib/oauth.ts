...
async function upsertIntegrationTokens(userId: string, provider: string, tokens: { access_token?: string; refresh_token?: string; expires_at?: number | null; scope?: string }) {
  const supabase = await createServerSupabase()
...
async function refreshGoogleToken(userId: string, integration: Integration) {
  ...
  if (data.access_token) {
    const expires_at = data.expires_in ? Math.floor(Date.now() / 1000) + Number(data.expires_in) : null
    await upsertIntegrationTokens(userId, "google", { access_token: data.access_token, refresh_token: integration.refresh_token, expires_at, scope: data.scope })
...
async function refreshMicrosoftToken(userId: string, integration: Integration) {
  ...
  if (data.access_token) {
    const expires_at = data.expires_in ? Math.floor(Date.now() / 1000) + Number(data.expires_in) : null
    await upsertIntegrationTokens(userId, "microsoft", { access_token: data.access_token, refresh_token: data.refresh_token ?? integration.refresh_token, expires_at, scope: data.scope ?? integration.scope })
    return { ...integration, access_token: data.access_token, refresh_token: data.refresh_token ?? integration.refresh_token, expires_at }
  }
...
export async function ensureValidToken(userId: string, provider: string) {
  ...
  if (integration.expires_at && integration.expires_at - 60 > now) {
    return integration
  }
...
export async function fetchEvents(userId: string, from?: string, to?: string) {
  // Fetch both providers if present and merge
  const supabase = await createServerSupabase()
  const { data: integrations } = await supabase.from("calendar_integrations").select("provider").eq("user_id", userId)
  ...
  for (const provider of providers) {
    ...
    if (provider === "google") {
      ...
      const timeMax = to ? new Date(to).toISOString() : new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toISOString()
      const url = `https://www.googleapis.com/calendar/v3/calendars/primary/events?timeMin=${encodeURIComponent(timeMin)}&timeMax=${encodeURIComponent(timeMax)}&singleEvents=true&orderBy=startTime`
      const res = await fetch(url, { headers: { Authorization: `Bearer ${integration.access_token}` } })
      if (!res.ok) continue
      const data = await res.json();
      (data.items ?? []).forEach((it: any) => {
        const start = it.start?.dateTime ?? it.start?.date
        events.push({ id: it.id, title: it.summary ?? "(Ingen tittel)", date: toDateKey(start), provider: "google" })
      })
    }

    if (provider === "microsoft") {
      const start = from ? new Date(from).toISOString() : new Date().toISOString()
      const end = to ? new Date(to).toISOString() : new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toISOString()
      ...
      (data.value ?? []).forEach((it: any) => {
        const start = it.start?.dateTime ?? it.start?.date
        events.push({ id: it.id, title: it.subject ?? "(Ingen tittel)", date: toDateKey(start), provider: "microsoft" })
      })
    }
  }
...
export async function revokeIntegration(userId: string, provider: string) {
  const supabase = await createServerSupabase()
  await supabase.from("calendar_integrations").delete().eq("user_id", userId).eq("provider", provider)
}

// Create Google watch channel for push notifications. Requires a publicly reachable webhook URL.
...
// Create Microsoft Graph subscription for events notifications. Expects WEBHOOK_BASE_URL.
export async function createMicrosoftSubscription(accessToken: string) {
  try {
    const webhookBase = process.env.WEBHOOK_BASE_URL
    if (!webhookBase) return null
    // expiration max typically a few days; here we set 3 days from now.
